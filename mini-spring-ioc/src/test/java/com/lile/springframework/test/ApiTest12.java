...
import com.lile.springframework.aop.framework.Cglib2AopProxy;
import com.lile.springframework.aop.framework.JdkDynamicAopProxy;
...
public class ApiTest12 {
    ...
    public void test_aop() throws NoSuchMethodException {
        AspectJExpressionPointcut pointcut = new AspectJExpressionPointcut("execution(* com.lile.springframework.test.bean.UserService12.*(..))");
    ...
    public void test_dynamic() {
        ...
        IUserService12 proxy_jdk = (IUserService12) new JdkDynamicAopProxy(advisedSupport).getProxy();
        // 测试调用
        System.out.println("测试结果：" + proxy_jdk.queryUserInfo());

        // 代理对象(Cglib2AopProxy)
        IUserService12 proxy_cglib = (IUserService12) new Cglib2AopProxy(advisedSupport).getProxy();
        // 测试调用
        System.out.println("测试结果：" + proxy_cglib.register("小芳"));
    }

    @Test
    public void test_proxy_class() {
        IUserService12 userService = (IUserService12) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), new Class[]{IUserService12.class}, (proxy, method, args) -> "你被代理了！");
